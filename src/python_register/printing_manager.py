...
class Printer:
    ...
    def print_receipt(
        self, receipt_type, items, sale_info, cash_tend=None, cc_tend=None
    ):
        ...
        for sublist in items:
            self.state_manager.cursor.execute(
                """SELECT item_name FROM inventory WHERE item_id = ?""", (sublist[3],)
            )
    ...
    def run_x(
        ...
        ending_date=datetime.combine(datetime.today(), time.max),
        event=None,
    ):
        ...
        for key, category in {
            ...
            self.state_manager.cursor.execute(
                """SELECT "%s" FROM sales WHERE sale_date >= ? AND sale_date <= ? AND is_voided != 1"""
                % (category),
                (
                    starting_date,
                    ending_date,
            ...
            sum_in_question = sum(
                Decimal(row[category]) for row in self.state_manager.cursor.fetchall()
            )
            ...
            self.printer.textln(
                self._line(
                    f"{key.split()[0]} Collected:",
                    f"${sum_in_question:.2f}",
                    self.config.data["printing_width"],
                )
            )

        propane_results = self.state_manager.cursor.execute(
            """SELECT price_at_sale, quantity FROM sale_items JOIN sales ON sale_items.sale_id = sales.sale_id WHERE sale_items.item_id IN (78, 79, 92, 692, 693, 1274) AND sales.sale_date >= ? AND sales.sale_date <= ?""",
            (
                starting_date,
                ending_date,
            ),
        ...
        propane_sold = (
            sum(row[0] * row[1] for row in propane_results).quantize(Decimal("0.01"))
            if propane_results
            else Decimal("0.00")
        )
        self.printer.textln(
            self._line(
                "Propane Sold:", f"${propane_sold}", self.config.data["printing_width"]
            )
        )

        self.printer.textln(
            self._line(
                "Gross Total", f"${gross_total:.2f}", self.config.data["printing_width"]
            )
        )
        ...
        self.printer.textln(
            self._line(
                "Gross Total w/o Tax",
                f"${gross_total_wo_tax:.2f}",
                self.config.data["printing_width"],
            )
        )

        self.state_manager.cursor.execute(
            """SELECT total FROM sales WHERE total < 0 AND sale_date BETWEEN ? AND ?""",
        ...
        if results == []:
            ...
            total_returns = sum(row["total"] for row in results).quantize(
                Decimal("0.01")
            )
            self.printer.textln(
                self._line(
        ...
        times_pressed = self.state_manager.cursor.execute(
            """SELECT SUM(times_pressed) FROM no_sale WHERE date >= ? AND date <= ?""",
            (
                starting_date,
                ending_date,
            ),
        ).fetchone()["SUM(times_pressed)"]
        self.printer.textln(
            self._line(
                "# Times No Sale:",
                f"{str(times_pressed)}",
    ...
    def print_receipt_header(
        ...
        if receipt_type == "void":
            ...
            self.printer.textln(
                f'{"Void Transaction":-^{self.config.data["printing_width"]}}'
            )
            self.printer.textln(("-" * self.config.data["printing_width"]))
            self.printer.textln(
                self._line(
            ...
            self.printer.textln(
                f"{'-' * 12} {datetime.now().strftime('%Y-%m-%d-%H:%M')} {'-' * 12}"
            )
            self.printer.textln("-" * 42)
            self.printer.textln(
                f"{'-' * 10}From: {starting_date.strftime('%Y-%m-%d-%H:%M')}{'-' * 10}"
            )
            self.printer.textln(
                f"{'-' * 11}To: {ending_date.strftime('%Y-%m-%d-%H:%M')}{'-' * 11}"
            )
            self.printer.textln("-" * 42)
            self.printer.ln(2)
