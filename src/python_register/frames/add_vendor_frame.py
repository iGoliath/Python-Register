...
class AddVendorFrame(BaseFrame):
    ...
    def build_widgets(self):
        ...
        tk.Label(self, text="Please enter item's vendor:", 
        ...
        self.add_vendor_listbox = tk.Listbox(
            self, height=3, font=("Arial", 50))
        add_vendor_scrollbar = tk.Scrollbar(
            self, orient=tk.VERTICAL, width=80
        )
        ...
        list_vendors = sorted([vendors[i]['vendor_name'] for i in range(0, len(vendors))])
        ...
        tk.Button(
            ...
            command = lambda: self.on_add_vendor_listbox_next()).grid(
    ...
    def on_show(self, reentering = False):
        ...
        if reentering:
            self.add_vendor_back_button.config(command = lambda: self.controller.reenter_back_button())
