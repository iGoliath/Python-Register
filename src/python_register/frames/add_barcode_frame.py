...
class AddBarcodeFrame(BaseFrame):
    ...
    def build_widgets(self):
        ...
        tk.Label(
            self, text="Please enter item's barcode:", font=("Arial", 50), width=25
        ...
        self.add_barcode_back_button.grid(column=0, row=0, sticky="ew", padx=(0, 15))
        tk.Button(
            back_quit_frame,
            text="Quit",
            font=("Arial", 50),
        ...
        tk.Button(
            ...
            text="Lookup Item",
            font=("Arial", 50),
            command=lambda: self.wm.show_frame("lookup_items", mode="additem"),
        ).grid(column=1, row=4, sticky="ew", pady=15)

    def on_show(self, reentering=False):

        self.add_barcode_entry.focus_set()
        if reentering:
            self.add_barcode_back_button.config(
