...
class AddCategoryFrame(BaseFrame):
    ...
    def build_widgets(self):
        ...
        tk.Label(
            self, text="Please enter item's category:", font=("Arial", 50), width=25
        ).grid(column=1, row=0, sticky="ew")

        self.add_category_listbox = tk.Listbox(self, height=4, font=("Arial", 45))
        add_category_scrollbar = tk.Scrollbar(self, orient=tk.VERTICAL, width=80)
        ...
        list_categories = sorted(
            [categories[i]["category_name"] for i in range(0, len(categories))]
        )
        ...
        tk.Button(
            ...
            text="Next",
            font=("Arial", 50),
            command=lambda: self.on_add_category_listbox_next(),
        ...
        self.add_category_back_button.grid(column=0, row=0, sticky="ew", padx=(0, 15))
        tk.Button(
            back_quit_frame,
            text="Quit",
            font=("Arial", 50),
    ...
    def on_show(self, reentering=False):
        ...
        if reentering:
            self.add_category_back_button.config(
