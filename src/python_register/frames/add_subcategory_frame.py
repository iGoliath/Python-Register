...
class AddSubcategoryFrame(BaseFrame):
    ...
    def build_widgets(self):
        ...
        tk.Label(
            self, text="Please enter item's subcategory:", font=("Arial", 50), width=25
        ).grid(column=1, row=0, sticky="ew")

        self.add_subcategory_listbox = tk.Listbox(self, height=4, font=("Arial", 45))
        add_subcategory_scrollbar = tk.Scrollbar(self, orient=tk.VERTICAL, width=80)

        self.add_subcategory_listbox.grid(column=1, row=1, sticky="nwe")
        add_subcategory_scrollbar.grid(column=1, row=1, sticky="nse")

        self.add_subcategory_listbox.config(
            yscrollcommand=add_subcategory_scrollbar.set
        )
        ...
        tk.Button(
            ...
            text="Next",
            font=("Arial", 50),
            command=lambda: self.on_add_subcategory_listbox_next(),
    ...
    def on_show(self, reentering=False):
        ...
        if reentering:
            self.add_subcategory_back_button.config(
    ...
    def populate_listbox(self):
        ...
        if subcategories == []:
            ...
            list_subcategories = sorted(
                [
                    subcategories[i]["category_name"]
