...
class Transaction:
    ...
    def complete_transaction(self, coupon_info=None):
        ...
        try:
            ...
            max_sale_id = self.db_cursor.fetchone()["MAX(sale_id)"]
            for key in self.items_list.keys():
                self.db_cursor.execute(
                    "INSERT OR IGNORE INTO sale_items VALUES(?, ?, ?, ?)",
                    (
                        max_sale_id,
                        self.items_list[key]["item_price"],
                        Dec4(self.items_list[key]["quantity_sold"]),
                        self.items_list[key]["item_id"],
                    ),
                )
                self.db_cursor.execute(
                    "SELECT item_quantity FROM inventory WHERE item_barcode = ?", (key,)
                )
                current_quantity = self.db_cursor.fetchone()["item_quantity"]
                if not self.returning:
                    self.db_cursor.execute(
                        "UPDATE inventory SET item_quantity = ? WHERE item_barcode = ?",
                        (
                            Dec4(
                                current_quantity - self.items_list[key]["quantity_sold"]
                            ),
                            key,
                        ),
                    )
        ...
        if coupon_info:
            self.db_cursor.execute(
                ...
                (max_sale_id, coupon_info[0], coupon_info[1]),
            )
    ...
    def complete_as_decrement(self):
        ...
        try:
            ...
            max_decrement_id = self.db_cursor.execute(
                """SELECT MAX(decrement_id) FROM inventory_decrements"""
            ).fetchone()["MAX(decrement_id)"]
            for key in self.items_list.keys():
                self.db_cursor.execute(
                    "INSERT OR IGNORE INTO inventory_decrements_items VALUES (?, ?, ?)",
                    (
                        max_decrement_id,
                        self.items_list[key]["item_id"],
                        Dec4(self.items_list[key]["quantity_sold"]),
                ...
                self.db_cursor.execute(
                    "UPDATE inventory SET item_quantity = ? WHERE item_barcode = ?",
                    (
                        Dec4(current_quantity - self.items_list[key]["quantity_sold"]),
                        key,
                    ),
                )
            self.db_conn.commit()
    ...
    def sell_item(self, entered_barcode, decimal_amount=Decimal("1")):
        ...
        results = self.db_cursor.execute(
            """SELECT item_name, item_price, item_taxable, item_id FROM inventory WHERE item_barcode = ?""",
            (entered_barcode,),
        ).fetchone()
        ...
        return (
            self.total,
            self.items_list[entered_barcode]["item_name"],
            self.items_list[entered_barcode]["item_price"] / Decimal(100),
            self.items_list[entered_barcode]["item_taxable"],
        )
